...
pd_arr      *pd_arr_copy(pd_arr* array)
{
    ...
    if (array->type == PD_T_ARR)
    {
        ...
        pd_mem_qcpy(new_array->val, array->val, sizeof(float) * array->len);
    else if (array->type == PD_T_SIZE_T)
        pd_mem_qcpy(new_array->val, array->val, sizeof(size_t) * array->len);
    else if (array->type == PD_T_CHAR)
        pd_mem_qcpy(new_array->val, array->val, sizeof(char) * array->len);
    else
