...
pd_csv   pd_get_test_csv_1(void)
{
    ...
    tmp = pd_csv_new_col(pd_arrSNew(T_STR, 4, col3), pd_strSNew("Zizi"));
    csv_test.begin = tmp;

    float col1[4] = {200.77777, 1, -1, -200};
    tmp->next = pd_csv_new_col(pd_arrSNew(T_FLOAT, 4, col1), pd_strSNew("Qi"));
    tmp = tmp->next;

    pd_char_a col2[4] = {pd_strSNew("Timothée"), pd_strSNew("Sylvain"), pd_strSNew("Paco"), pd_strSNew("Tony")};
    tmp->next = pd_csv_new_col(pd_arrSNew(T_STR, 4, col2), pd_strSNew("Name"));
    tmp = tmp->next;

    pd_char_a col4[4] = {pd_strSNew("Ca va"), pd_strSNew("Musclée"), pd_strSNew("Musclée"), pd_strSNew("Musclée")};
    tmp->next = pd_csv_new_col(pd_arrSNew(T_STR, 4, col4), pd_strSNew("Main droite"));
    tmp = tmp->next;
...
void    csv_with_conf()
{
    ...
    pd_csv_conf conf = pd_prep_init_conf(pd_arrSNew(T_STR, 1, del_1), pd_arrSNew(T_STR, 2, ohe_1));
...
void    pd_csv_manual()
{
    ...
    pd_str_a  del_str = pd_arrSNew(T_STR, 1, del_1);
    prep_delete(&csv_1, del_str);

    pd_char_a ohe_1[2] = {pd_strSNew("Main droite"), pd_strSNew("Zizi")};
    pd_str_a  ohe_str = pd_arrSNew(T_STR, 2, ohe_1);
