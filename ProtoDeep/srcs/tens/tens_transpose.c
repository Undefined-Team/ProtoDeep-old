...
static void         pd_coord_update(size_t *n_shape, size_t *coord, int *n_max, size_t *index)
{
    if (*coord == *n_shape)
    {
        *coord = 0;
        --coord;
        ++(*coord);
        pd_coord_update(n_shape - 1, coord, n_max - 1, index);
    }
    else
        *index += *n_max;
}
...
pd_tensor       *pd_tens_transpose_cpy(pd_tensor *tensor, pd_size_t_a *new_dim)
{
    ...
    while (tmp_len-- > 0)
    {
        ...
        *end_val-- = *(t_end_val - index);
        ++(*coord_x);
        pd_coord_update(n_shape, coord_x, n_max, &index);
    }
