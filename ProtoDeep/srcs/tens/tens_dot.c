...
pd_arr      *pd_tens_dot_shape_mult(pd_tensor *a, pd_tensor *b, pd_arr *axis, pd_arr *counter_axis, pd_arr *mem_shapes)
{
    ...
    size_t counter_axis_len = a->shape_len - ((pd_arr **)axis->val)[0]->len;
...
pd_tensor   *pd_tens_dot(pd_tensor *a, pd_tensor *b, pd_arr *axis)
{
    ...
    counter_axis = (tens_len > axis_len ? pd_tens_get_counter_axis(tens_len, axis) : NULL);
    ...
    pd_tensor *t_a = pd_tens_reshape(a, new_dim_a);
    pd_tensor *t_b = pd_tens_reshape(b, new_dim_b);
